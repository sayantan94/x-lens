...
async def run_platform(platform: str, config: SimulationConfig, profiles: list[AgentProfile],
    ...
    # --- Thesis rounds: bull and bear leads post structured cases ---
    bull_lead = None
    bear_lead = None
    for p in profiles:
        if p.archetype == "bull_lead_analyst":
            bull_lead = p
        elif p.archetype == "bear_lead_analyst":
            bear_lead = p

    if bull_lead or bear_lead:
        print(f"[{platform}] Starting thesis rounds...", file=sys.stderr)
        thesis_round = total_rounds + 1

        if _web_search_round_setter:
            _web_search_round_setter(thesis_round)

        # Bull lead posts thesis
        if bull_lead:
            agent = agent_graph.get_agent(bull_lead.user_id)
            if agent:
                bull_prompt = (
                    "You are posting your BULL THESIS SUMMARY. This is your most important post. "
                    "Use ALL available data — the OI data, market context, and price levels you were given "
                    "in your persona context, PLUS call search_web to get the latest real-time data "
                    "(current price, analyst targets, recent news, earnings). "
                    "Combine the seeded data with fresh web data to build the strongest fact-based bull case:\n\n"
                    "🐂 BULL THESIS: [ticker/topic]\n"
                    "DATA FROM CONTEXT:\n- [OI levels, positioning, max pain, call walls from your context]\n"
                    "DATA FROM WEB:\n- [current price, analyst targets, recent catalysts from search]\n"
                    "CATALYST: [what drives the move]\n"
                    "TARGET: [price/level with reasoning]\n"
                    "RISK: [what invalidates this thesis]\n"
                    "CONVICTION: [high/medium/low with reasoning]"
                )
                bull_action = ManualAction(
                    action_type=ActionType.CREATE_POST,
                    action_args={"content": bull_prompt},
                )
                await env.step({agent: bull_action})
                new_actions, last_rowid = fetch_new_actions_from_db(db_path, last_rowid, agent_names)
                for action in new_actions:
                    log_action(log_path, thesis_round, platform, action["agent_id"],
                               action["agent_name"], action["action_type"], action["action_args"])
                print(f"  [{platform}] Bull lead posted thesis", file=sys.stderr)

        # Bear lead posts thesis
        if bear_lead:
            agent = agent_graph.get_agent(bear_lead.user_id)
            if agent:
                bear_prompt = (
                    "You are posting your BEAR THESIS SUMMARY. This is your most important post. "
                    "Use ALL available data — the OI data, market context, and price levels you were given "
                    "in your persona context, PLUS call search_web to get the latest real-time data "
                    "(current price, analyst downgrades, risk factors, macro headwinds). "
                    "Combine the seeded data with fresh web data to build the strongest fact-based bear case:\n\n"
                    "🐻 BEAR THESIS: [ticker/topic]\n"
                    "DATA FROM CONTEXT:\n- [OI levels, put walls, skew, positioning from your context]\n"
                    "DATA FROM WEB:\n- [current price, risks, downgrades, macro headwinds from search]\n"
                    "RISK TO BULLS: [why the bull case is wrong, with data]\n"
                    "TARGET: [downside price/level with reasoning]\n"
                    "HEDGE: [how to protect]\n"
                    "CONVICTION: [high/medium/low with reasoning]"
                )
                bear_action = ManualAction(
                    action_type=ActionType.CREATE_POST,
                    action_args={"content": bear_prompt},
                )
                await env.step({agent: bear_action})
                new_actions, last_rowid = fetch_new_actions_from_db(db_path, last_rowid, agent_names)
                for action in new_actions:
                    log_action(log_path, thesis_round, platform, action["agent_id"],
                               action["agent_name"], action["action_type"], action["action_args"])
                print(f"  [{platform}] Bear lead posted thesis", file=sys.stderr)

        # Rebuttal round — let all agents react to the theses
        thesis_round += 1
        if _web_search_round_setter:
            _web_search_round_setter(thesis_round)

        all_ids = [p.user_id for p in profiles]
        actions = {}
        for aid in all_ids:
            agent = agent_graph.get_agent(aid)
            if agent:
        ...
        if actions:
            ...
            for action in new_actions:
                log_action(log_path, thesis_round, platform, action["agent_id"],
                           action["agent_name"], action["action_type"], action["action_args"])
            print(f"  [{platform}] Rebuttal round — {len(new_actions)} actions", file=sys.stderr)
